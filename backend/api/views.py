...
from .models import City, Room, User, Event, ActivityLog
from .serializers import CitySerializer, RoomSerializer, UserSerializer, EventSerializer, UserProfileSerializer, ActivityLogSerializer
...
def create_city(request):
    serializer = CitySerializer(data=request.data, context=serializer_context(request))
...
def get_cities(request):
    ...
    serializer = CitySerializer(cities, many=True, context=serializer_context(request))
    return Response(serializer.data, status=status.HTTP_200_OK)
...
def get_city_by_slug(request, slug):
    try:
        ...
        serializer = CitySerializer(city, context=serializer_context(request))
        return Response(serializer.data, status=status.HTTP_200_OK)
...
def create_room(request):
    serializer = RoomSerializer(data=request.data, context=serializer_context(request))
    if serializer.is_valid():
...
def get_rooms(request):
    ...
    serializer = RoomSerializer(rooms, many=True, context=serializer_context(request))
    return Response(serializer.data, status=status.HTTP_200_OK)
...
def get_room(request, room_id):
    try:
        ...
        serializer = RoomSerializer(room, context=serializer_context(request))
        return Response(serializer.data, status=status.HTTP_200_OK)
...
def update_room(request, room_id):
    ...
    serializer = RoomSerializer(room, data=request.data, partial=True, context=serializer_context(request))
    if serializer.is_valid():
        serializer.save()
        return Response(serializer.data, status=status.HTTP_200_OK)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def update_city(request, city_id):
    ...
    serializer = CitySerializer(city, data=data, partial=True, context=serializer_context(request))
    if serializer.is_valid():
        serializer.save()
        return Response(serializer.data, status=status.HTTP_200_OK)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def create_event(request):
    serializer = EventSerializer(data=request.data, context=serializer_context(request))
    if serializer.is_valid():
...
def get_events(request):
    ...
    serializer = EventSerializer(events, many=True, context=serializer_context(request))
    return Response(serializer.data, status=status.HTTP_200_OK)
...
def update_event(request, event_id):
    ...
    serializer = EventSerializer(event, data=request.data, context=serializer_context(request))
    if serializer.is_valid():
        serializer.save()
        return Response(EventSerializer(serializer.instance, context=serializer_context(request)).data, status=status.HTTP_200_OK)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def publish_event(request, event_id):
    try:
        ...
        event.save()
        serializer = EventSerializer(event, context=serializer_context(request))
        return Response(serializer.data, status=status.HTTP_200_OK)
    except Event.DoesNotExist:
        return Response({'error': 'Event not found'}, status=status.HTTP_404_NOT_FOUND)
...
# Role-filtered endpoints for content-creators
@api_view(['GET'])
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
