...
import { getCity, updateCity, deleteCity, requestReviewCity } from '../api/cities';
...
export default function EditCity() {
    ...
    useEffect(() => {
        const fetchAll = async () => {
            try {
                setLoading(true);
                const [cityRes, usersRes] = await Promise.all([getCity(id), getUsers()]);
                ...
                setFormData({
                    name: city.name || '',
                    meeting_url: city.meeting_url || '',
                    assistant: city.assistant || '',
                    content_creator_id: city.content_creator_id || '',
                    city_manager: city.city_manager || '',
                ...
                setStatus(city.status || 'draft');
                setOriginalData(city);
                
                // Handle new image format: {original, webp, avif}
                setPreviewUrls({
                    thumbnail: city.thumbnail?.original || null,
                    flyin_graphic: city.flyin_graphic?.original || null,
                    loop_graphic: city.loop_graphic?.original || null,
                    support_audio: city.support_audio_url || null,
                });
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            data.append('content_creator_id', formData.content_creator_id);
            data.append('city_manager', formData.city_manager);
            ...
            showToast(true, 'City updated successfully!');
        } catch (err) {
            console.error(err);
            showToast(false, 'Error updating city. Please try again.');
        } finally {
            setUpdating(false);
        }
    };
    ...
    const handleDelete = async () => {
        if (!window.confirm('Are you certain you want to delete this city forever?')) return;
        setDeleting(true);
        try {
            await deleteCity(id);
            navigate('/my-cities');
            ...
            console.error(err);
            showToast(false, 'Error deleting city.');
            setDeleting(false);
        }
    };
        ...
        <div className="max-w-4xl mx-auto py-6 sm:py-8 px-4 sm:px-6 relative animate-fade-up">
            ...
            {status === 'in_review' && (
                <div className="mb-6 p-4 rounded-xl flex items-center gap-3 text-sm font-medium" style={{ background: isDark ? 'rgba(59,130,246,0.15)' : 'rgba(59,130,246,0.1)', border: isDark ? '1px solid rgba(59,130,246,0.3)' : '1px solid rgba(59,130,246,0.2)', color: isDark ? '#93c5fd' : '#1e40af' }}>
                    <Info size={20} />
                    This city is currently In Review.
                </div>
            )}
            {status === 'published' && (
            ...
            <div className="rounded-2xl p-6 sm:p-8" style={cardStyle}>
                <h2 className="text-xl sm:text-2xl font-bold tracking-tight mb-1" style={{ color: headingColor }}>Edit City</h2>
                <p className="text-xs sm:text-sm mb-6" style={{ color: subColor }}>ID: {id}</p>

                <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label className="block text-[11px] font-bold tracking-wider uppercase mb-1.5" style={{ color: labelColor }}>City Name</label>
                            <input type="text" name="name" required value={formData.name} onChange={handleInputChange} disabled={isDisabled} style={{ ...inputStyle, opacity: isDisabled ? 0.6 : 1 }} />
                        </div>
                        <div>
                        ...
                        <div>
                            <label className="block text-[11px] font-bold tracking-wider uppercase mb-1.5" style={{ color: labelColor }}>City Manager</label>
                            <div style={{ opacity: isDisabled ? 0.6 : 1, pointerEvents: isDisabled ? 'none' : 'auto' }}>
                                <CustomSelect
                                    name="city_manager"
                                    value={formData.city_manager}
                                    onChange={handleInputChange}
                                    placeholder="Assign City Manager"
                                    isDark={isDark}
                                    options={usersList.filter(u => u.role === 'content-creator').map(u => ({ value: u.id || u.pk, label: u.name || u.username || u.email }))}
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="pt-2 flex items-center justify-between">
                        <div>
                            <label className="block text-[11px] font-bold tracking-wider uppercase mb-1.5" style={{ color: labelColor }}>City Status</label>
                            <p className="text-xs mt-1" style={{ color: subColor }}>{formData.is_active ? 'Active & Accessible' : 'Inactive & Hidden'}</p>
                        </div>
                        
                        <div 
                            className={`relative w-12 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors ${isDisabled ? 'opacity-50 pointer-events-none' : ''}`}
                            style={{ 
                                background: formData.is_active ? (isDark ? '#059669' : '#10b981') : (isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'),
                                boxShadow: isDark ? 'inset 1px 1px 3px rgba(0,0,0,0.4)' : 'inset 1px 1px 3px rgba(0,0,0,0.2)'
                            }}
